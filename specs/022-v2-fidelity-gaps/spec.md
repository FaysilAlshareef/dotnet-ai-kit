...
- Q: How is the `dotnet-ai` hook-execution gap fixed — change the hook command, or fix install? → A: **[NEEDS CLARIFICATION: prefer `${CLAUDE_PLUGIN_ROOT}`-relative invocation of a bundled launcher, vs. requiring `dotnet tool install --global DotnetAiKit.Tool` + documenting/PATH-ordering away the stale v1 shim, vs. invoking via `dotnet <dll>`].** Default assumption for planning: make the hook robust without a global install (plugin-root-relative), and additionally document removing the stale shim.
...
- **FR-022-10**: The generated `hooks.json` command MUST resolve to the v2 hook backend at fire time on a supported install, NOT to a stale v1 `dotnet-ai` Python shim. [NEEDS CLARIFICATION: plugin-root launcher vs. required global tool install vs. `dotnet <dll>`.]
