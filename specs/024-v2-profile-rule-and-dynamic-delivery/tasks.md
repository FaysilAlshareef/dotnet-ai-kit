...
- [ ] T037 [US5] Project MCP config per host from a TARGET-FACING descriptor via a shared writer in `src/DotnetAiKit.Hosts/` (new `McpProjection.cs` + per-projector calls). **BLOCKED on decision D-MCP** — the root `.mcp.json` is the kit's own dev config (`codebase-memory-mcp`) and MUST NOT be projected into users' repos; the MCP track is meant to point at the kit's own server (feature 029). Options: (a) author an `artifacts/` descriptor for the future `dotnet-ai mcp serve`, marked not-yet-available; (b) build the mechanism + a documented placeholder; (c) descope MCP to 029. Resolve before starting T037.
