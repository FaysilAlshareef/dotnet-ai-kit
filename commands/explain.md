...
| `--preview` | Same as default behavior (explain is read-only) |
...
Explain is inherently read-only. `--preview` and `--dry-run` behave identically to the default -- they display the explanation. No files are ever written or modified.
