...
def test_constitution_universal_whitelist_is_5_rules() -> None:
    ...
    assert "5 universal" in text, "Constitution must declare '5 universal' rules per FR-011"

    for rule_name in EXPECTED_UNIVERSAL_RULES:
        assert rule_name in text, f"Constitution must mention '{rule_name}' as a universal rule"
...
def test_constitution_total_is_16_rules() -> None:
    ...
    assert "11 path-scoped" in text, (
        "Constitution must declare 11 path-scoped rules"
        " (was 12 before v1.0.8 / async-concurrency promotion)"
    )
